...
class AttendanceController extends Controller
{
    ...
    private function defaultLetterContent(string $scheduleStartTime = '8:30 AM', array $violations = []): array
    {
        ...
        if (!empty($violations['missedLogs'])) {
            $actionItems[] = count($actionItems) + 1 . ". For missing biometric logs, please provide the time and location where you were last seen, along with proof of attendance (such as supervisor certification, completed work output, or other verifiable evidence).";
        }
 ...
 */
    ...
    private function generateWordDocument(array $allEmployeeData)
    {
        ...
        foreach ($allEmployeeData as $index => $emp) {
            ...
            if (count($v['lateAM']) > 0) {
                $sh = $section->addParagraph();
                $sh->addText('Late Arrivals — Morning (' . $s['totalLateAM'] . ' ' . ($s['totalLateAM'] == 1 ? 'instance' : 'instances') . ')', $sectionHead);
                $sh->setParagraphStyle(['spaceAfter' => 60, 'borderBottomSize' => 6, 'borderBottomColor' => 'cbd5e1']);
                ...
                foreach ($v['lateAM'] as $i => $row) {
                    ...
                    $cs = $i % 2 === 1 ? $tdAltStyle : $tdCellStyle;
                    $tr->addCell(2000, $cs)->addText($row['dateFormatted'], $bodyStyle);
                    $tr->addCell(1800, $cs)->addText($row['timeIn'] ?? '—', $bodyStyle);
            ...
            if (count($v['latePM']) > 0) {
                $sh = $section->addParagraph();
                $sh->addText('Late Returns — Afternoon (' . $s['totalLatePM'] . ' ' . ($s['totalLatePM'] == 1 ? 'instance' : 'instances') . ')', $sectionHead);
                $sh->setParagraphStyle(['spaceAfter' => 60, 'borderBottomSize' => 6, 'borderBottomColor' => 'cbd5e1']);
            ...
            if (count($v['missedLogs']) > 0) {
                ...
                foreach ($v['missedLogs'] as $i => $row) {
                    ...
                    $tr->addCell(1200, $cs)->addText($row['timeInAM']    ?? '—', !$row['timeInAM']    ? $redBold : $bodyStyle);
                    $tr->addCell(1200, $cs)->addText($row['timeOutLunch'] ?? '—', !$row['timeOutLunch'] ? $redBold : $bodyStyle);
                    $tr->addCell(1200, $cs)->addText($row['timeInPM']    ?? '—', !$row['timeInPM']    ? $redBold : $bodyStyle);
                    $tr->addCell(1200, $cs)->addText($row['timeOutPM']   ?? '—', !$row['timeOutPM']   ? $redBold : $bodyStyle);
            ...
            if (count($v['undertime'] ?? []) > 0) {
                ...
                foreach ($v['undertime'] as $i => $row) {
                    ...
                    $cs = $i % 2 === 1 ? $tdAltStyle : $tdCellStyle;
                    $tr->addCell(2000, $cs)->addText($row['dateFormatted'], $bodyStyle);
                    $tr->addCell(1800, $cs)->addText($row['timeOut'] ?? '—', $bodyStyle);
                    $tr->addCell(2000, $cs)->addText($row['timeStr'], $bodyStyle);
                }
                $section->addTextBreak(1);
            }
            ...
            if (count($v['missedLogs'] ?? []) > 0)
                $actionItems[] = 'For missing biometric logs, please provide the time and location where you were last seen, along with proof of attendance (such as supervisor certification, completed work output, or other verifiable evidence).';
