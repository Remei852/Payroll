...
class AttendanceService
{
    ...
    private function processEmployeeLogs(string $employeeCode, Carbon $date, $logs): void
    {
        ...
        AttendanceRecord::updateOrCreate(
            ...
            [
                ...
                'total_late_minutes' => $isWorkingDay ? ($lateMinutesAM + $lateMinutesPM) : 0,
    ...
    private function assignLogsToTimeSlots(array $logs, Carbon $date): array
    {
        ...
        foreach ($correctedLogs as $log) {
            ...
            if ($log['type'] === 'IN') {
                ...
                elseif ($totalMinutes >= 840 && !$slots['lunch_in'] && $slots['morning_in']) {
                    $slots['lunch_in'] = $log['time'];
                }
                ...
                // Lunch OUT: 11:00 AM - 12:30 PM (first OUT before 12:30)
                if ($totalMinutes >= 660 && $totalMinutes < 750 && !$slots['lunch_out']) {
    ...
    private function inferLogTypesFromTime($logs, Carbon $date): array
    {
        ...
        foreach ($filtered as $index => $log) {
            ...
            elseif ($index === 2 && $logCount === 3) {
                // 3 logs: IN(morning) - OUT(lunch) - ?(third)
                // If third log is in lunch-return window (12:45-14:00) → it's lunch IN, OUT PM is missing
                // If third log is after 14:00 → it's afternoon OUT
                if ($totalMinutes >= $lunchBoundary && $totalMinutes < 840) {
                    $inferredType = 'IN'; // lunch return — OUT PM will be missing
                } else {
                    $inferredType = 'OUT'; // afternoon departure
                }
            }
            elseif ($index === 2) {
    ...
    private function determineAttendanceStatus(
        ...
        $hasMorningSlots = !empty($timeSlots['morning_in']);

        $isAbsent = (empty($pairs) || $firstIn === null) && !$hasPmSlots;
        ...
        // Half Day AM = only PM slots present (no morning_in)
        // Half Day PM = has morning + lunch_in but NO afternoon_out (left without clocking out PM)
        $hasMorningPresence  = !empty($timeSlots['morning_in']);
        $hasLunchIn          = !empty($timeSlots['lunch_in']);
        $hasAfternoonOut     = !empty($timeSlots['afternoon_out']);
        $hasAfternoonPresence = $hasAfternoonOut || $hasLunchIn;

        // No morning at all → Half Day AM
        $isHalfDayAM = !$hasMorningPresence && $hasAfternoonPresence;

        // Has morning + came back from lunch (lunch_in) but never clocked OUT PM → Half Day PM
        $isHalfDayPM = $hasMorningPresence && $hasLunchIn && !$hasAfternoonOut;

        // Has morning but never came back at all (no lunch_in, no afternoon_out) → Half Day PM
        $isHalfDayPMNoReturn = $hasMorningPresence && !$hasLunchIn && !$hasAfternoonOut;

        $isHalfDay = $isHalfDayAM || $isHalfDayPM || $isHalfDayPMNoReturn;
    ...
    private function calculateWorkdayRendered(string $status, float $totalWorkedHours, WorkSchedule $schedule): float
    {
        ...
        $isHalfDay = in_array('half day', $statusParts)
                  || in_array('half day am', $statusParts)
                  || in_array('half day pm', $statusParts);
    ...
    public function getAttendanceSummary(): array
    {
        ...
        foreach ($records as $employeeId => $employeeRecords) {
            ...
            $halfDayCount = $employeeRecords->filter(fn($r) =>
                str_contains($r->status ?? '', 'Half Day')
            )->count();
