...
function CashAdvancesTab({ employeeId }) {
    ...
    return (
        <div className="space-y-4">
            <div>
                <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Cash Advance History</p>
                <p className="mt-1 text-xs text-slate-400 italic">This tab shows the history of deductions. New advances are managed in the Payroll section.</p>
            </div>
            ...
            {loading ? (
                ...
                <div className="rounded-lg border border-dashed border-slate-300 px-4 py-8 text-center text-xs text-slate-500">
                    No cash advances recorded for this employee.
                </div>
            ) : (
                <div className="space-y-4">
                    {/* Pending */}
                    {pending.length > 0 && (
                        <div>
                            <p className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-amber-600">Pending Deduction ({pending.length})</p>
                                ...
                                {pending.map(a => (
                                    <div key={a.id} className="flex items-center justify-between rounded-lg border border-amber-100 bg-amber-50 px-3 py-2">
                                        <div>
                                            ...
                                            <p className="text-xs text-amber-600">Released: {fmtDate(a.release_date)} {a.reason && `· ${a.reason}`}</p>
                                        </div>
                                        <span className="rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-semibold text-amber-700">Unpaid</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                                    ...
                                    <div key={a.id} className="flex items-center justify-between rounded-lg border border-slate-200 bg-white px-3 py-2">
                                        <div>
                                            ...
                                            <p className="text-xs text-slate-400">
                                                Released: {fmtDate(a.release_date)} · {a.reason || 'No reason'} · Deducted {fmtDate(a.deducted_at)}
                                                {a.payroll_period && (
                                        ...
                                        <span 
                                            className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${
                                                a.payroll_period?.status === 'CLOSED'
                                                    ? 'bg-green-100 text-green-700'
                                                    : 'bg-blue-100 text-blue-700'
                                            }`}
                                        >
                                            {a.payroll_period?.status === 'CLOSED' ? 'Deducted' : 'Pending Finalization'}
                                        </span>
                                    </div>
                                ))}
                            </div>
                    ...
                    <div className="fixed inset-y-0 right-0 z-30 flex w-full max-w-lg flex-col border-l border-slate-200 bg-white shadow-xl">
                        ...
                        >
                            <div className="flex-1 overflow-y-auto px-5 py-5">
                                                            ...
                                                            <div className="grid grid-cols-2 gap-3">
                                                                ...
                                                                <div>
                                                                    ...
                                                                    <input
                                                                        ...
                                                                        placeholder="0.00"
                                                                    />
                                                                    <p className="mt-1 text-[10px] text-slate-400 italic leading-tight">Enter the full monthly amount. The system will automatically split this across each payroll (e.g., 50% for semi-monthly).</p>
                                                                </div>
                                ...
                                {activeTab === 'advances' && selectedEmployeeId && (
                                    <CashAdvancesTab employeeId={selectedEmployeeId} />
                                )}
                            </div>
